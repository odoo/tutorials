...
class SubProductWizard(models.TransientModel):
    ...
    line_ids = fields.One2many(
        "sub.product.wizard.line", "wizard_id", string="Sub Products"
    )
    ...
    def default_get(self, fields_list):
        ...
        if sale_order_line_id:
            ...
            sale_sub_products = []
            default_kit_products = sale_order_line.product_template_id.sub_product_ids
            for product in default_kit_products:
                sale_sub_products.append(
                    (
                        0,
                        0,
                        {
    ...
    def action_confirm(self):
        ...
        existing_sub_lines = sale_order.order_line.filtered(
            lambda l: l.is_sub_product and l.linked_line_id.id == sale_order_line.id
        )
        existing_sub_product_map = {
            line.product_id.id: line for line in existing_sub_lines
        }
        sub_total_price = 0
        for line in self.line_ids:
            if line.product_id.id in existing_sub_product_map:
                ...
                existing_line.write(
                    {"product_uom_qty": line.quantity, "price_unit": 0.0}
                )
            else:
        ...
        for product_id, existing_line in existing_sub_product_map.items():
            if product_id not in self.line_ids.mapped("product_id").ids:
                existing_line.with_context(from_wizard=True).unlink()
