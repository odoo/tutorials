...
class BillEntryWizard(models.TransientModel):
    ...
    line_ids = fields.One2many('bill.entry.details', 'wizard_id', string='Product Lines')
    ...
    custom_duty_journal_entry_id = fields.Many2one(
        'account.move',
        string='Custom Duty Journal Entry',
        readonly=True,
        related='move_id.custom_duty_journal_entry_id',
    )
    custom_duty_journal_entry_lines = fields.One2many(
        related='move_id.custom_duty_journal_entry_id.line_ids',
    ...
    def action_create_and_post_journal_entry(self):
        for record in self:
            ...
            if not record.line_ids:
                raise UserError(_("Journal Entry Can't be created without Bill Entry Detail Line."))
            if record.line_ids:
            ...
            journal_entry = self.env['account.move'].create({
                'move_type': 'entry',  # i.e. Journal Entry
