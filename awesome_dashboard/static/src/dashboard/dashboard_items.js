import { registry } from "@web/core/registry";
import { NumberCard } from "./cards/number_card";
import { PieChartCard } from "./cards/piechart_card";

const items = [{
    id: "average_quantity",
    description: "Average amount of t-shirt",
    Component: NumberCard,
    size: 1.3,
    props: (data) => ({
        title: "Average amount of t-shirt by order this month",
        value: data.average_quantity
    })
},
{
    id: "average_time",
    description: "Average order time",
    Component: NumberCard,
    size: 2,
    props: (data) => ({
        title: "Average time for an order to go from \'new\' to \'sent\' this month",
        value: data.average_time
    })
},
{
    id: "nb_new_orders",
    description: "Amount of new orders",
    Component: NumberCard,
    props: (data) => ({
        title: "Number of new orders this month",
        value: data.nb_new_orders
    })
}, {
    id: "nb_cancelled_orders",
    description: "Amount of cancelled order",
    Component: NumberCard,
    size: 1.3,
    props: (data) => ({
        title: "Number of cancelled order this month",
        value: data.nb_cancelled_orders
    })
}, {
    id: "total_amount",
    description: "Total ammount of t-shirt",
    Component: NumberCard,
    size: 1.3,
    props: (data) => ({
        title: "Total amount of new orders this month",
        value: data.total_amount
    })
},
{
    id: "orders_by_size",
    description: "Shirts orders by size",
    Component: PieChartCard,
    size: 2,
    props: (data) => ({
        title: "Shirts orders by size",
        value: data.orders_by_size
    })
}
]

registry.category("awesome_dashboard").add("dashboard_items", items);
