...
class EstateBooking(models.Model):
    ...
    @api.depends("final_price", "booking_percentage", "invoice_ids.amount_total", "invoice_ids.amount_residual", "invoice_ids.payment_state")
    def _compute_amounts_and_status(self):
        ...
        for record in self:
            ...
            total_paid = sum(posted_invoices.mapped(lambda m: m.amount_total - m.amount_residual))
            record.total_paid_amount = total_paid
            record.remaining_amount = record.final_price - total_paid

            if total_paid == 0:
                record.payment_status = "pending"
            elif float_compare(total_paid, record.final_price, precision_rounding=0.01) >= 0:
                record.payment_status = "fully_paid"
            elif float_compare(total_paid, record.booking_amount, precision_rounding=0.01) == 0:
