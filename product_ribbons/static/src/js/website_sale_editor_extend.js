...
options.registry.WebsiteSaleProductsItem = options.registry.WebsiteSaleProductsItem.extend({
    ...
    async _saveRibbon(isNewRibbon = false) {
        ...
        const position = (this.$ribbon.hasClass('o_ribbon_left') || this.$ribbon.hasClass('o_tag_left')) ? 'left' : 'right';
        const style = this.$ribbon.attr('data-style') || 'ribbon';
    ...
    async _setRibbon(ribbonId) {
        ...
        // Select all elements that reference the same ribbon ID.
