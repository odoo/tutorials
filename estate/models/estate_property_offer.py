...
class EstatePropertyOffer(models.Model):
    ...
    deadline_date = fields.Date(
        compute="_compute_validity", inverse="_inverse_validity"
    )
    ...
    _sql_constraints = [
        (
            "check_offer_price",
            "CHECK(price > 0)",
            "Offer price can not be less than or equal to zero",
        )
    ]
    ...
    def _compute_validity(self):
        for record in self:
            if record.created_date:
                record.deadline_date = record.created_date + timedelta(
