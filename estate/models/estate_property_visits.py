...
class EstatePropertyVisits(models.Model):
    ...
    _description = "Estate Property Visits"
    ...
    def _onchange_sales_person(self):
        Visit = self.env['estate.property.visits']
        assigned_ids = Visit.search([]).mapped('sales_person.id')
        ...
        for record in self:
            ...
            is_assigned = Visit.search_count(
    ...
    def _check_time_slot(self):
        for record in self:
            ...
            if self.env['estate.property.visits'].search_count(domain) > 0:
                raise UserError(
