...
class EstateOffer(models.Model):
    ...
    _sql_constraints = [
        (
            "check_offer_price",
            "CHECK (price > 0)",
            "Offer price should strictly be positive",
        )
    ]
    ...
    def _compute_date_deadline(self):
        ...
        for record in self:
            start_date = record.create_date if record.create_date else datetime.now()
        ...
        for vals in vals_list:
            ...
            if vals.get("price") is not None:
                ...
                if vals["price"] < highest_offer:
                    raise UserError(
                        f"The offer price must be higher than the existing offer of {highest_offer}."
                    )
