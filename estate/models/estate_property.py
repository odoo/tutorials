...
class EstateProperty(models.Model):
    ...
    salesperson_id = fields.Many2one('res.users',string='Sales Person',default=lambda self: self.env.user,required=True)
    ...
    _sql_constraints = [
        ...
        ('check_bedrooms_positive', 'CHECK(bedrooms >= 0)', 'The number of bedrooms must be zero or positive.'),
    ]
    ...
    def _compute_best_price(self):
        for property in self:
            property.best_price = max(property.offer_ids.mapped('offer_price'), default=0.0)
