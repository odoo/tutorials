...
class EstateProperty(models.Model):
    ...
    _sql_constraints = [
        (
            ...
            "CHECK(expected_price >= 0.00)",
            "The Expected price must be Positive.",
        ),
        (
            "check_selling_price",
            "CHECK(selling_price >= 0.00)",
        ...
        for record in self:
            if record.selling_price == 0.00:
                ...
                raise ValidationError(
                    f"Selling price cannot be lower than 90% of the expected price."
                )
    ...
    def _unlink_except_new_or_cancelled(self):
        for record in self:
            if record.state not in ["new", "cancelled"]:
                raise UserError(
                    f'Cannot delete property "{record.name}" because its state is "{record.state}". Only properties in "New" or "Cancelled" state can be deleted.'
                )
    ...
    def action_set_property_cancel(self):
        ...
        self.message_post(body=f"Property {self.name} has been cancelled.")
