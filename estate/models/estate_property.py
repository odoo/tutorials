...
class Property(models.Model):
    ...
    def _check_selling_price_close_to_expected(self):
        for record in self:
            if not (
                tools.float_utils.float_is_zero(
                    record.selling_price, precision_digits=2
                )
                or tools.float_utils.float_compare(
                ...
                raise exceptions.ValidationError(
                    "The selling price must be at least 90% of the expected price. Lower the expected price, or find a better offer"
                )
