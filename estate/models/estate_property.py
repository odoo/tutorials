...
class EstateProperty(models.Model):
    ...
    def _compute_best_price(self):
        for property in self:
            ...
            property.best_price = max(prices) if prices else 0
    ...
    def action_sold(self):
        for property in self:
            if 'accepted' not in property.offer_ids.mapped('status'):
    ...
    def _check_selling_price(self):
        for property in self:
            if property.selling_price != 0:
                difference_in_price = float_compare(property.selling_price, property.expected_price*90/100, precision_digits=0)
