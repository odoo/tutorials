...
class PropertiesController(http.Controller):
    ...
    @http.route(
        ["/properties", "/properties/<int:page>"],
        type="http",
        auth="public",
        website=True,
    )
    def properties(self, page=1, **kwargs):
        ...
        properties = request.env["estate.property"].search(
            [("status", "not in", ["Sold", "Cancelled"]), ("active", "=", "False")],
    ...
    @http.route(
        ["/property", "/property/<int:property_id>"],
        type="http",
        auth="public",
        website=True,
    )
    def get_property_details(self, property_id=None, **kwargs):
        ...
        return request.render(
            "estate.template_property_details", {"prop_details": property}
        )
