...
class EstateWebsite(http.Controller):
    @route(['/properties',
           '/properties/page/<int:page>'], auth='public', website=True)
    ...
    @route(['/properties/<model("estate.property"):property>'], auth='public', website=True)
    def property_detail(self, property, **kwargs):
        if not property or property.state in ['sold', 'canceled'] or not property.active:
