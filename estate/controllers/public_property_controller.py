...
class EstatePropertyController(http.Controller):
    ...
    def list_properties(self, page=1, **kwargs):
        ...
        properties = request.env['public.property'].sudo().search(
            domain, order='create_date desc', offset=offset, limit=properties_per_page)

        total_properties = request.env['public.property'].sudo().search_count([
    ...
    def property_detail(self, property_id, **kwargs):
        property_obj = request.env['public.property'].sudo().browse(property_id)
