...
class EstateTestCase(TransactionCase):
    ...
    def test_creation_area(self):
        self.assertEqual(
            self.properties[0].total_area, 150, "Total area should be the sum of garden area and living area"
        )

    def test_action_sell(self):
        self.properties[0].action_sold_property()
        self.assertEqual(
            self.properties[0].state, "sold", "The property state should update on pressing sell button"
        )
    
    def test_add_offer_for_sold_property(self):
        self.properties[0].write({"state": "sold"})
        with self.assertRaises(UserError, msg="You cannot create offer for a sold property"):
    ...
    def test_sell_a_property_with_no_accepted_offers(self):
        ...
        accepted_offers = self.properties[0].offer_ids.filtered(lambda offer: offer.status == 'accepted')
        self.assertFalse(accepted_offers, "There should be no accepted offers")

        with self.assertRaises(UserError, msg="Expected UserError when selling without accepted offers"):
            self.properties[0].action_sold_property()
