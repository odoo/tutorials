...
class SaleOrderLine(models.Model):
    ...
    kit_component_ids = fields.One2many(
        "sale.order.line", "parent_line_id", string="Kit Components"
    )
    parent_line_id = fields.Many2one(
        "sale.order.line", string="Parent Kit Line", ondelete="cascade"
    )
    display_price_unit = fields.Float(
        compute="_compute_display_price_unit", readonly=False
    )
