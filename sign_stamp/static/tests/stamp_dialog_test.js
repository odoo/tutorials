...
import { LogoAndStampDialog } from "@sign_stamp/dialogs/stamp_dialog";
...
QUnit.module("Stamp Dialog", function (hooks) {
    const mountLogoAndStampDialog = async () => {
        ...
        await mountInFixture(LogoAndStampDialog, target, {
            props: {
                signature: {
                    name,
                    company,
    ...
    QUnit.test("stamp dialog renders correctly", async function (assert) {
        ...
        assert.strictEqual(
            target.querySelector('input[name="company"]').value,
            company,
            "Should auto-fill the company"
        );
        assert.strictEqual(
            target.querySelector('input[name="address"]').value,
            address,
            "Should auto-fill the address"
        );
        assert.strictEqual(
            target.querySelector('input[name="city"]').value,
            city,
            "Should auto-fill the city"
        );
        assert.strictEqual(
            target.querySelector('input[name="country"]').value,
            country,
            "Should auto-fill the country"
        );
        assert.strictEqual(
            target.querySelector('input[name="vat"]').value,
            vat,
            "Should auto-fill the vat"
        );
    ...
    QUnit.test(
        "stamp dialog toggles active class on frame input change",
    ...
    QUnit.test(
        "stamp dialog default font",

        async function (assert) {
            const mountLogoAndStampDialogSaved = async () => {
                const mockRPC = async (route) => {
                    if (route.includes("/web/sign/get_fonts/")) {
                        assert.step(route);
                        return [];
            ...
            const hasGroup = async () => true;
            patchUserWithCleanup({ hasGroup });
            await mountLogoAndStampDialogSaved();
    ...
    QUnit.test(
        "stamp dialog draw mode does not allow to submit sign with no sign drawn",
        async function (assert) {
            await mountLogoAndStampDialog();
