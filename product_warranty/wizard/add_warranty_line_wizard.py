...
class AddWarrantyLine(models.TransientModel):
    ...
    wizard_id = fields.Many2one("add.warranty.wizard", string="Wizard", ondelete="cascade")
    product_id = fields.Many2one("product.product", string="Product", required=True)
    order_line_id = fields.Many2one("sale.order.line", required=True, ondelete="cascade")
    warranty_period = fields.Integer(string="Warranty Period (Years)", related="warranty_id.year", store=True)
    warranty_id = fields.Many2one("product.warranty",string="Select Warranty")
    ...
    def create(self, vals):
        """Ensure product_id is always set"""
